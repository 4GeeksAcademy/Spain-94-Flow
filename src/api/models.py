...
class Businesses(db.Model):
    ...
    business_postal_code: Mapped[str] = mapped_column(String(10), nullable=False)
...
class Services(db.Model):
    ...
    name: Mapped[str] = mapped_column(String(75), unique=True, nullable=False)
...
class Clients(db.Model):
    ...
    def serialize_client(self):
        return {
            ...
            "business_id": self.business_id,
            "services": (
                [service.serialize_service() for service in self.services]
                if self.services
                else []
            ),
            "notes": [note.serialize_note() for note in self.notes],
            "payments": (
                [payment.serialize_payment() for payment in self.payments]
                if self.payments
                else []
            ),
            "appointments": (
            ...
            "service_history": (
                [history.serialize_history() for history in self.service_history]
            ...
            "service_instances": (
                [instance.serialize() for instance in self.service_instances]
...
class Notes(db.Model):
    ...
    service_history = relationship("ServiceHistory", back_populates="note")
...
class Payments(db.Model):
    ...
    estimated_total: Mapped[int] = mapped_column(Numeric(10, 2), nullable=False)
    payments_made: Mapped[int] = mapped_column(
        Numeric(10, 2), nullable=False, default=0
    )
    payment_date: Mapped[Date] = mapped_column(Date, nullable=True)
    status: Mapped[str] = mapped_column(
        Enum("pending", "paid", name="status_enum"), nullable=False, default="pending"
    )
...
class Appointments(db.Model):
    ...
    calendar = relationship("Calendar", back_populates="appointment", uselist=False)
    service_history = relationship(
    ...
    def serialize_appointment(self):
        return {
            ...
            "client_services": (
                [service.serialize_service() for service in self.client.services]
...
class ServiceHistory(db.Model):
    ...
    appointment = relationship("Appointments", back_populates="service_history")
...
class ClientService(db.Model):
    ...
    client = relationship(
        "Clients", back_populates="service_instances", overlaps="services,clients"
    )
    service = relationship(
        "Services", back_populates="client_instances", overlaps="clients,services"
    )
